...
class TestGetMeanEnergyAtFinal:
    def test_extracts_correct_values(self):
        rows = _make_rows("viroid_v0", "rich", range(100, 105), step=500, alive=10, mean_energy=0.55)
    ...
    def test_selects_final_step_only(self):
        rows = _make_rows("viroid_v0", "rich", range(100, 102), step=250, alive=10, mean_energy=0.30)
        rows += _make_rows("viroid_v0", "rich", range(100, 102), step=500, alive=10, mean_energy=0.55)
        result = get_mean_energy_at_final(rows, "viroid_v0", "rich")
        assert len(result) == 2
        assert all(v == pytest.approx(0.55) for v in result)
...
class TestAnalyzeMeanEnergySupplement:
    def _build_two_condition_rows(
        ...
        rows = _make_rows(cond_a, harshness, range(100, 120), step=500, alive=10, mean_energy=energy_a)
        rows += _make_rows(cond_b, harshness, range(100, 120), step=500, alive=10, mean_energy=energy_b)
        return rows

    def test_h1_returns_results_for_all_harshness(self):
        rows = []
        for h in ["rich", "medium", "sparse", "scarce"]:
            rows += self._build_two_condition_rows("viroid_v0", 0.6, "viroid_v0v1", 0.4, h)
    ...
    def test_h2_returns_results_for_all_harshness(self):
        rows = []
        for h in ["rich", "medium", "sparse", "scarce"]:
            rows += self._build_two_condition_rows("viroid_v0v1v2v3", 0.8, "viroid_v0v1v2", 0.3, h)
        results = analyze_mean_energy_supplement(rows)
    ...
    def test_clear_separation_gives_large_delta(self):
        rows = self._build_two_condition_rows("viroid_v0", 0.9, "viroid_v0v1", 0.1)
        # Only need the 4 harshness for H1+H2 — fill others with dummy data
        for h in ["medium", "sparse", "scarce"]:
            rows += self._build_two_condition_rows("viroid_v0", 0.9, "viroid_v0v1", 0.1, h)
            rows += self._build_two_condition_rows("viroid_v0v1v2v3", 0.9, "viroid_v0v1v2", 0.1, h)
        rows += self._build_two_condition_rows("viroid_v0v1v2v3", 0.9, "viroid_v0v1v2", 0.1, "rich")
        results = analyze_mean_energy_supplement(rows)
        h1_rich = [r for r in results if r["hypothesis"] == "H1_energy" and r["harshness"] == "rich"][0]
