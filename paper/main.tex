...
\title{Semi-Life: A Capability Ladder from Replicator to Life}

\author{Yuya Mukai$^{1}$ \\
\mbox{} \\
$^1$Mukai Entertainment \\
mukaiyuya@gmail.com}
...
The replicator-to-life transition is a cost-then-benefit trajectory where each
capability is justified only when coupled with sufficient internal infrastructure.
\end{abstract}

Submission type: \textbf{Full Paper}\\
Data/Code available at: \url{https://github.com/TheIllusionOfLife/life_transition}\\
\blfootnote{\textcopyright 2026 Yuya Mukai. Published under a Creative
...
We adopt a \emph{functional analogy framework}: a computational capability is
a functional analogy of a biological criterion if and only if (a) it requires
sustained resource consumption, (b) its removal causes measurable population
degradation, and (c) it forms a feedback loop with at least one other criterion.
The background platform (Section~\ref{sec:background}) implements all seven
textbook criteria---cellular organisation, metabolism, homeostasis, growth,
reproduction, response to stimuli, and evolution---as interdependent dynamic
processes validated through criterion-ablation tests.
...
replicator-to-life transition using a pre-existing ALife world as the background
...
The background world is a seven-criterion ALife system: a population of
swarm-based organisms in a continuous 2D environment, each instantiating
cellular organisation, metabolism, homeostasis, growth, reproduction,
response to stimuli, and evolution as interdependent dynamic processes.
...
decline, and (c)~cross-criterion feedback loops.
...
collection (pre-registration document available at
\url{https://github.com/TheIllusionOfLife/life_transition/blob/main/docs/research/preregistration.md}).
...
at all harshness levels (JT $p\!\statPHfourRich{}$); the V3 recovery dominates the
...
The phase diagrams (Figure~\ref{fig:phase}) operationalise the replicator-to-life
...
is significant at all harshness levels ($p\!\statPHsevenRich{}$): the V3 metabolic
...
non-monotonic intermediate dip means the replicator-to-life transition is better
...
\url{https://github.com/TheIllusionOfLife/life_transition}
(software DOI: \url{https://doi.org/10.5281/zenodo.19397251}).
Raw TSV data files and statistical output (JSON) are archived on
Zenodo at \url{https://doi.org/10.5281/zenodo.19397240}.
