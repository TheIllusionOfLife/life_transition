...
ablations. Ablations are applied at initialization (step~0), not mid-run;
organisms never experience criterion loss after operating normally.
...
We further quantify coupling via intervention-based causal effects
...
high ($h^2 \approx 0.99$): given point mutation rate 0.01 per gene and
scale 0.1, mutation variance ($V_m = 0.026$) is small relative to
standing genetic variation ($V_g = 3.19$, measured as genome diversity
at the final step). Second, genome drift trajectories diverge
...
zero. Third, under cyclic resource stress (period=2,000 steps), evolved
populations exhibit faster per-cycle recovery rates
($\bar{r}_{\text{evo}} = 0.068$ vs.\
$\bar{r}_{\text{no\text{-}evo}} = 0.037$; pooled $p = 0.010$;
Figure~\ref{fig:evo_evidence}B), with the advantage persisting across
successive cycles, consistent with adaptive tuning of resource
...
ecological strategies, we run 5,000-step simulations ($n$=10 seeds)
...
Organisms present in both snapshots of a pair ($n$=1,262 shared)
...
the adjusted Rand index between paired windows is low (ARI=0.04--0.09),
...
effect ($\delta$=0.34) observed in single-ablation experiments.
...
(B)~Per-cycle recovery rates under cyclic resource stress: evolved
populations recover faster after each low-resource phase (pooled $p$=0.010).}
