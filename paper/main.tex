...
mean: 293.1 (median: 294, IQR: 283--305). All $p$-values Holm-Bonferroni
...
No Boundary     & 121.8 & $-$58.4 & 4.36 & 1.00 & $<$0.001 & $^{***}$ \\
No Growth       & 185.6 & $-$36.7 & 5.34 & 0.97 & $<$0.001 & $^{***}$ \\
No Evolution    & 278.3 & $-$5.1  & 0.57 & 0.18 & 0.016 & $^{*}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Pairwise ablation synergy scores ($n$=30 per condition).
Negative synergy indicates sub-additive interaction (shared failure
pathways). $\Delta$ values as percentage of baseline ($\bar{x}$=322).
\emph{Note:} Pairwise experiments use the default Toy metabolism
(baseline $\bar{x}$=322) whereas Table~\ref{tab:ablation} uses Graph
metabolism (baseline $\bar{x}$=293); cross-table $\Delta$ comparisons
...
\midrule
(Metab, Homeo)    & 278.8 & 528.3 & $-$249.5 & 0.53 \\
(Metab, Response)  & 276.5 & 558.3 & $-$281.8 & 0.50 \\
(Reprod, Growth)   & 290.4 & 418.5 & $-$128.1 & 0.69 \\
(Boundary, Homeo)  & 268.7 & 509.6 & $-$240.9 & 0.53 \\
(Response, Homeo)  & 277.5 & 534.8 & $-$257.3 & 0.52 \\
(Reprod, Evol)     & 290.4 & 313.9 & $-$23.5  & 0.93 \\
\bottomrule
\multicolumn{5}{l}{\footnotesize Ratio = $\Delta_{AB}$ / Expected; values $<$1 indicate shared pathways.}
\end{tabular}
\end{table}

\paragraph{Growth--reproduction confound.}
The (reproduction, growth) pair confirms: $\Delta_{AB} =
\Delta_{\text{reproduction}}$ = 290.4, so growth's effect is fully mediated
...
$\delta$=0.18) reflects the 2000-step simulation horizon.
To demonstrate evolution's contribution at longer timescales, we run two
additional experiments:

\paragraph{Long run.}
Extending simulations to 10,000 steps ($n$=30) yields $d$=1.46, Cliff's
...
$d$=1.02, Cliff's $\delta$=0.57, $p < 0.001$), demonstrating that
...
The modest 2000-step evolution effect ($d$=0.57) grows to $d$=1.46 at
10,000 steps and $d$=1.02 under environmental perturbation, confirming
that evolutionary adaptation accumulates across generations.

\subsection{Limitations}

\textbf{Growth} uses a maturation toggle rather than a full developmental
program; a morphogenetic model would strengthen this criterion's
functional-analogy claim.
\textbf{Evolution} reaches $d$=1.46 at 10,000 steps but demonstrating
...
corrected), with Cliff's $\delta$ ranging from 0.18 to 1.00.
