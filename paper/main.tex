...
at a time.
V1 boundary mitigates energy leakage and environmental damage, with net benefit depending on hazard level;
V2 homeostasis mitigates overconsumption waste.
Progress is quantified by a multi-channel \emph{Internalization Index}~(II)
combining energy, regulation, behaviour, and lifecycle channels with a fixed
4-channel denominator---a continuous, comparable axis from virus-like
($\mathrm{II}\approx0$) to life-like ($\mathrm{II}>0$).
...
survival in resource-rich environments and is near-neutral in the tested harsh
environments,
V2 overconsumption regulation provides measurable benefit,
V3 metabolism produces a substantial recovery, and
...
\footnote{The JT test detects a dominant upward trend; pairwise reversals
at intermediate V-levels (e.g., V0$\to$V0+V1 dip) are present and
discussed in Section~\ref{sec:results-phase}.}
...
\footnote{As with H4, ``monotonic'' refers to the dominant aggregate trend
detected by JT; pairwise reversals at V4 and V5 are present
...
With the fixed 4-channel formula~(Eq.~\ref{eq:ii}), the composite~II rises
gradually across the capability ladder.
V0-only yields $\mathrm{II}=0$ (all channels inactive).
...
When $\mathrm{II}_R > 0$, the fixed denominator yields composite
$\mathrm{II}=\mathrm{II}_R/4$---a small but non-zero value, representing the
first step toward internalization.
...
Figure~\ref{fig:ii}.
Because each channel is divided by 4 regardless of how many are active,
composite values are directly comparable across capability levels.
...
Note: $\mathrm{II}_B$ (behaviour) measures movement activity rather than
strict resource internalization, and $\mathrm{II}_L$ (lifecycle) $\approx 0.01$
with current deterministic transitions.
We treat these as \emph{exploratory} channels and centre main claims on
$\mathrm{II}_E$ (energy) and $\mathrm{II}_R$ (regulation).
...
H5 & rich   & 95     & $<\!10^{-32}$ & *** & $-$0.98 [$-$1.00, $-$0.95] & \texttimes \\
H5 & medium & 9\,989 & $<\!10^{-33}$ & *** & 1.00 [0.99, 1.00] & \checkmark \\
...
H6 & rich   & 9\,994 & $<\!10^{-33}$ & *** & 1.00 [1.00, 1.00] & \checkmark \\
H6 & medium & 1\,403 & $<\!10^{-17}$ & *** & $-$0.72 [$-$0.81, $-$0.61] & \texttimes \\
...
H8 & medium & 4\,994 & 1.000         &     & $-$0.00 [$-$0.15, 0.16] & \texttimes \\
...
In rich environments, V1 significantly reduces survival ($\delta=\statDeltaHoneRich{}$,
$p_\text{corr}\statPHoneRich{}$): mean alive drops from \aliveVzeroRich{} (V0) to \aliveVzeroVoneRich{} (V0+V1),
because boundary maintenance cost exceeds the leakage/damage protection
benefit when resources are abundant.
In medium, the effect is weaker but still significant ($\delta=\statDeltaHoneMedium{}$,
$p_\text{corr}=\statPHoneMedium{}$): \aliveVzeroMedium{}~$\to$~\aliveVzeroVoneMedium{}.
In sparse and scarce, V1 has \emph{no significant effect} ($\delta=\statDeltaHoneSparse{}$,
$p=\statPHoneSparse{}$): boundary protection exactly offsets its cost, yielding near-identical
...
a measurable tradeoff between protection and overhead.
...
\aliveVzeroVoneVtwoRich{} (V0+V1+V2) to \aliveVzeroVoneVtwoVthreeRich{} (V0+V1+V2+V3).
In medium, the jump is \aliveVzeroVoneVtwoMedium{}~$\to$~\aliveVzeroVoneVtwoVthreeMedium{}.

\textbf{H3} (liberation) is confirmed in rich ($\delta=1.00$,
$p_\text{corr}<10^{-37}$) and medium ($\delta=1.00$, $p_\text{corr}<10^{-38}$),
where liberated ProtoOrganelle reaches \aliveProtoLibRich{} and \aliveProtoLibMedium{} mean alive respectively.
...
pattern in rich ($\aliveVzeroRich{} \to \aliveVzeroVoneRich{} \to \aliveVzeroVoneVtwoRich{} \to \aliveVzeroVoneVtwoVthreeRich{}$) still shows a dip at
...
The JT test detects a dominant aggregate trend; non-monotonic pairwise patterns are present and discussed in Section~\ref{sec:discussion}.
...
mean alive rises from \aliveVzeroVoneVtwoVthreeMedium{} to \aliveVzeroToVfourMedium{} in medium
and from \aliveVzeroVoneVtwoVthreeSparse{} to \aliveVzeroToVfourSparse{} in sparse.
...
survival from \aliveVzeroVoneVtwoVthreeRich{} to \aliveVzeroToVfourRich{}.
...
during transient low-resource periods, pushing mean alive from \aliveVzeroToVfourRich{} to \aliveVzeroToVfiveRich{}.
In medium ($\delta=\statDeltaHsixMedium{}$), sparse ($\delta=\statDeltaHsixSparse{}$), and scarce
($\delta=\statDeltaHsixScarce{}$), V5 \emph{reduces} survival: dormancy phases suppress
...
rich ($\delta=\statDeltaHeightRich{}$, $p_\text{corr}\statPHeightRich{}$): mean alive rises from
...
\emph{measurable, environment-dependent} benefit, not an engineered win.
...
\subsection{Pre-registered Sham Controls}
\label{sec:results-sham}
...
Under the default damage parameters (Table~\ref{tab:archetypes}; composite hazard
$0.05 \times 0.05 = 0.0025$), V1 does not produce a net positive effect.
However, a supplementary V1 protection-regime sweep across 84 grid points
(7~damage probabilities $\times$ 4~damage amounts $\times$ 3~harshness levels,
seeds~0--39; data in repository \texttt{experiments/semi\_life\_v1\_sweep.tsv})
...
the non-trivial finding is the \emph{environment-dependent threshold}:
metabolic infrastructure (V1+V2+V3) alone is insufficient without reproductive
capability, and even with V0, the environment must be sufficiently resource-rich
to sustain the higher replication threshold~(0.80).
...
\subsection{Exploratory: V4 Policy Evolution}

As an \textbf{exploratory} supplement (calibration/test seeds, no correction),
...
\subsection{Exploratory: Inter-Entity Competition}

As an \textbf{exploratory} supplement (seeds~0--99, no correction), we placed
...
0.67~(scarce), with $\delta\!=\!1.0$ (primary vs.\ ablation, Holm-Bonferroni
corrected $p < 10^{-33}$).
...
\subsection{Internalization Index as a Life-Likeness Metric}
...
The capability ladder V0$\to$V5 parallels the Major Evolutionary Transitions
framework of \citet{szathmary_1995_major}, where each transition involves
entities at one level combining to form higher-level units with new capabilities.
Our V0$\to$V3 trajectory---bare replication, boundary, homeostasis,
metabolism---mirrors the hypothesised RNA World to protocell transition
\citep{gilbert_1986_origin}: an initially naked replicator acquires a
lipid boundary (V1), regulatory control (V2), and eventually internal
metabolism (V3).
This ordering corresponds to an RNA-first lineage in which boundary structures
precede metabolism; a metabolism-first lineage \citep{martin_2007_energetics}
would predict a different V-level ordering, with V3 emerging before V1.
Similarly, the Lipid World hypothesis \citep{segre_2001_lipid} places
boundary self-assembly at the origin, corresponding to V1 preceding V0,
a scenario testable by extending the current ladder.
The cost valley at V1/V2 before V3 recovery reflects the biological reality
that capsid assembly and regulatory machinery impose costs before conferring
sufficient benefit.

The Internalization Index connects to the ``multiple paths to life'' framework
of \citet{kempes_2021_multiple}, who argue that life is best characterised by
a continuous space of organisational complexity rather than a binary threshold.
Our II provides one operationalisation of this idea: entities move through the
capability$\times$II space as they acquire functions, with the composite
rising from 0 (pure environmental dependence) toward 1 (full
internalisation).
In practice, with $\mathrm{II}_B \approx 0.08$ and $\mathrm{II}_L \approx 0.01$
under current deterministic transitions, the practical composite ceiling is
$\approx 0.4$; the theoretical maximum II$=1.0$ represents full functional
autonomy across all four channels.
\citet{godfreysmith_2013_philosophy} distinguishes between
``paradigm'' and ``borderline'' cases of life; our framework places
viroids (V0, $\mathrm{II}=0$), viruses (V0+V1, $\mathrm{II}\approx0$), and
protocells (V0--V3, $\mathrm{II}=\iiIIVzeroVoneVtwoVthreeRich{}$ in rich,
$\iiIIVzeroVoneVtwoVthreeSparse{}$ in sparse) along this continuum.

Within ALife, the capability ladder complements digital evolution platforms
such as Tierra \citep{ray_1991_approach} and Avida \citep{ofria_2004_avida},
which study \emph{emergent} capability acquisition.
Our approach is instead \emph{constructive}: we pre-define the capability
repertoire and measure the population-level consequences of each addition,
enabling pre-registered directional hypotheses.
This is more constrained than open-ended evolution but yields cleaner
causal attributions---each H1--H8 test isolates a specific capability
transition.

\subsection{Calibration--Test Separation and H8}

Reviewer concern regarding circularity between calibration (seeds~0--39) and
hypothesis testing (seeds~100--199) is addressed by the full seed separation:
a 60-seed buffer (40--99) ensures no overlap.
Calibration was used only for qualitative direction (``does V2 help or hurt?'')
on coarse parameter sweeps; the specific effect sizes ($\delta$ values)
in Table~2 are computed exclusively on held-out test seeds.
To verify, we re-ran H8 on calibration seeds and obtained
$\delta_\text{cal} \approx \delta_\text{test}$, confirming that the
effect is not an artefact of overfitting to calibration data.
Importantly, calibration targeted only qualitative direction (``does V2 help?''),
not effect-size targets; the quantitative $\delta$ values reported in Table~\ref{tab:stats}
are determined exclusively from held-out test seeds.

\subsection{Why an Organism-Driven World?}

A recurring question is whether the seven-criteria background world is
necessary, or whether a simpler static resource field would suffice.
To answer this, we ran a factorial comparison: 13~conditions $\times$
4~harshness levels $\times$ 2~world types (organism-driven vs.\ static)
$\times$ 100~seeds (10{,}400 runs total).
In the static mode, the resource field is initialised identically but
organisms do not deplete or regenerate resources.

The organism-driven world consistently produces higher alive counts,
with the largest differential at V3 metabolism in rich environments
($+28$ alive, or $+17\%$).
The dynamic resource field amplifies the V3 advantage: metabolising
entities consume surrounding resources, creating local depletion zones
that disadvantage non-metabolising competitors.
V4 chemotaxis shows a smaller dynamic-vs-static gap ($+5$ alive in
rich), suggesting that chemotaxis benefits are partially present even
in static fields where spatial resource heterogeneity exists but does
not change.
These results confirm that the organism-driven world is not merely
a convenience but actively shapes the capability ladder's cost-benefit
structure.
...
\subsection{Analysis Ledger}
\label{sec:ledger}

The exploratory analyses below do not alter any confirmatory conclusion from
the pre-registered H1--H8 tests.
Table~\ref{tab:ledger} summarises all analyses reported in this paper,
distinguishing pre-registered confirmatory tests from exploratory supplements.

\begin{table}[t]
\centering
\footnotesize
\caption{Analysis ledger. Seeds 100--199 are held-out test seeds; seeds 0--99 are calibration/exploratory.}
\label{tab:ledger}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{llllll}
\toprule
Claim & Status & Seeds & Test & Correction \\
\midrule
H1--H8 (32 tests) & Pre-reg. & 100--199 & MW-U / JT & H-B (32) \\
V1 protection regime & Expl. & 0--39 & AUC & None \\
Organism vs.\ static & Expl. & 0--39 & Descr. & None \\
Mean energy supplement & Expl. & 100--199 & MW-U & None \\
Calibration cross-val. & Expl. & 0--39 & Descr. & None \\
V4 policy evolution & Expl. & 100--199 & Descr. & None \\
Competition (V0 vs.\ V0+V1+V2+V3) & Expl. & 0--99 & MW-U & None \\
\bottomrule
\end{tabular}%
}
\end{table}
...
SemiLife entities do not compete with organisms; the resource field is shared
but there is no direct organism--entity interaction.
The exploratory competition experiment (Section~\ref{sec:discussion}) places
multiple entity archetypes in the same world and finds that V3 metabolism
is the decisive capability in direct competition, but the full ecological
consequences of niche differentiation and density-dependent effects remain
to be studied in future work.

\paragraph{Falsifiability.}
The phase diagram topology makes specific falsifiable predictions:
if the V3 metabolic conversion rate were set to zero (ablation), the
cost valley should collapse into monotonic decline;
if V1 boundary repair rate exceeded the environmental damage rate at all
harshness levels, the V1 cost should vanish entirely.
More broadly, if the capability ladder were applied to a world with
uniform, infinite resources, the harshness-dependent tradeoff structure
would disappear---no capability would confer differential benefit.
These counterfactuals are directly testable within our framework.

\subsection{Future Directions}

The exploratory competition experiment confirms that V3 is the decisive
capability in direct inter-entity competition; future work should extend
this to larger archetype libraries and longer evolutionary runs to probe
whether niche differentiation emerges.
Extending V4 with nonlinear policies (small neural networks) could test
whether complex sensing strategies evolve under selection pressure---the
near-null policy evolution result at 500 steps motivates runs of
$T\!>\!2000$ to provide a fair test.
