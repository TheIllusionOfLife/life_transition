...
A factorial comparison (13~conditions $\times$ 4~harshness $\times$
2~world types $\times$ 100~seeds) confirms the organism-driven world
yields consistently higher alive counts than a static resource field.
The V3 advantage is largest ($+28$ alive, $+17\%$ in rich environments):
metabolising entities deplete local resources, disadvantaging
non-metabolisers.
V4 chemotaxis shows a smaller dynamic-vs-static gap ($+5$ alive),
as gradient-following benefits persist in static spatial heterogeneity.
The organism-driven world actively shapes the capability ladder's
